    ...
    <Layout>
      ...
      <section className="relative h-[50vh] min-h-[400px] flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0">
          <img
            src={heroImage}
            alt="Contact Gentlemen Excursions"
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <a
                  ...
                  className="flex items-center gap-4 p-5 border border-gold/30 rounded-lg hover:border-gold hover:bg-gold/5 transition-all group"
                >
                ...
                {/* Email */}
                <a
                  href="mailto:hello@gentlemen-excursions.com"
                  className="flex items-center gap-4 p-5 border border-zinc-800 rounded-lg hover:border-gold/50 transition-all group"
                >
                ...
                <div className="flex gap-3">
                  {['facebook', 'instagram', 'pinterest'].map((social) => (
                    <a
                      key={social}
                      href="#"
                      className="w-12 h-12 rounded-full border border-zinc-800 flex items-center justify-center hover:border-gold hover:text-gold transition-colors text-zinc-500"
                    >
                      {social === 'facebook' && (
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                  ...
                  <div className="relative">
                    ...
                    <Select value={formData.destination} onValueChange={handleDestinationChange}>
                      <SelectTrigger className="w-full bg-transparent border-0 border-b border-zinc-700 rounded-none focus:border-gold focus:ring-0 py-3 text-white text-lg h-auto">
                        <SelectValue placeholder="Sélectionnez une destination" />
                      </SelectTrigger>
                      <SelectContent className="bg-[#0A0A0A] border border-gold/30">
                        {tours.map((tour) => {
                          const excursionData = t.excursions[tour.titleKey];
                          return (
                            <SelectItem 
                              key={tour.id} 
                              value={tour.id}
                              className="text-white hover:bg-gold/10 focus:bg-gold/10 focus:text-gold"
                            >
                              {excursionData?.title || tour.titleKey}
                            </SelectItem>
                          );
                        })}
                        <SelectItem 
                  ...
                  <div className="relative">
                    ...
                    <textarea
                      ...
                      required
                      rows={5}
                      className="w-full bg-transparent border-b border-zinc-700 focus:border-gold outline-none py-3 text-white text-lg resize-none transition-colors placeholder:text-zinc-600"
                      placeholder="Décrivez votre projet d'aventure..."
                    />
                  </div>
                  ...
                  <button
                    ...
                    className="w-full group inline-flex items-center justify-center gap-3 bg-gold text-black px-10 py-5 text-sm font-semibold uppercase tracking-wider hover:bg-gold/90 transition-all disabled:opacity-50 disabled:cursor-not-allowed rounded-lg"
                  >
                    {isSubmitting ? (
                      <>
